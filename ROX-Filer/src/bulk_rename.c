...
void bulk_rename(const char *dir, GList *items)
{
	...
	gtk_widget_set_tooltip_text(button, _("Make the New column a copy of Old"));

	dialog_add_icon_button(GTK_DIALOG(box), ROX_ICON_CANCEL,
			_("_Cancel"), GTK_RESPONSE_CANCEL);

	button = button_new_mixed(ROX_ICON_EXECUTE, _("_Rename"));
	gtk_widget_set_can_default(button, TRUE);
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(box))),
				hbox, FALSE, TRUE, 0);
	...
	gtk_widget_set_tooltip_text(replace_entry, _("This is a regular expression to search for.\n"
	...
	gtk_widget_set_tooltip_text(with_entry, _("The first match in each filename will be replaced "
			"by this string. "
	...
	gtk_widget_set_tooltip_text(button, _("Do a search-and-replace in the New column. "
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(box))), swin, TRUE, TRUE, 0);
...
static gboolean rename_items(const char *dir, GtkListStore *list)
{
	...
	do {
		...
		if (after[0] == '/')
			...
			dest = (const char *) make_path(dir, after);
	...
	if (slash_example)
	{
		...
		if (!confirm(message, ROX_ICON_EXECUTE, "Rename anyway"))
		{
			g_free(message);
			goto fail;
		}
