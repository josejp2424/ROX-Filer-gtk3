...
#include <gtk/gtk.h>
#include <gtk/gtkx.h>  /* GtkSocket: GTK3 X11 embedding API */
#include <gdk/gdkx.h>
...
struct _PanelIcon {
	...
	Panel		*panel;
	GtkWidget *widget;	/* Event box for the icon */
...
#define PANEL_ICON(obj) \
	((PanelIcon *) G_TYPE_CHECK_INSTANCE_CAST((obj), panel_icon_get_type(), PanelIcon))
...
static void panel_icon_refresh_visual(PanelIcon *pi);
...
static gboolean draw_panel_edge(GtkWidget *widget, cairo_t *cr,
				Panel *panel);
...
Panel *panel_new(const gchar *name, PanelSide side)
{
	...
	rox_window_set_wmclass(GTK_WINDOW(panel->window), "ROX-Panel", PROJECT);
	...
	/* The viewport uses GTK3 alignment, expansion and margin properties. */
	align = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
	gtk_container_add(GTK_CONTAINER(panel->window), align);

	vp = gtk_viewport_new(NULL, NULL);
	gtk_widget_set_halign(vp,
		panel->side == PANEL_RIGHT ? GTK_ALIGN_END :
		panel->side == PANEL_LEFT ? GTK_ALIGN_START : GTK_ALIGN_FILL);
	gtk_widget_set_valign(vp,
		panel->side == PANEL_BOTTOM ? GTK_ALIGN_END :
		panel->side == PANEL_TOP ? GTK_ALIGN_START : GTK_ALIGN_FILL);
	gtk_widget_set_hexpand(vp,
		panel->side == PANEL_TOP || panel->side == PANEL_BOTTOM);
	gtk_widget_set_vexpand(vp,
		panel->side == PANEL_LEFT || panel->side == PANEL_RIGHT);
	if (panel->side == PANEL_TOP)
		gtk_widget_set_margin_bottom(vp, EDGE_WIDTH);
	else if (panel->side == PANEL_BOTTOM)
		gtk_widget_set_margin_top(vp, EDGE_WIDTH);
	else if (panel->side == PANEL_LEFT)
		gtk_widget_set_margin_end(vp, EDGE_WIDTH);
	else
		gtk_widget_set_margin_start(vp, EDGE_WIDTH);
	gtk_viewport_set_shadow_type(GTK_VIEWPORT(vp), GTK_SHADOW_NONE);
	gtk_container_add(GTK_CONTAINER(align), vp);

	g_signal_connect(align, "draw",
			 G_CALLBACK(draw_panel_edge), panel);

	if (side == PANEL_TOP || side == PANEL_BOTTOM)
	{
		panel->adj = gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(vp));
		box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
		panel->before = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
		panel->after = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
	}
	else
	{
		panel->adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(vp));
		box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
		panel->before = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
		panel->after = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
	}
	...
	if (side == PANEL_TOP || side == PANEL_BOTTOM)
		gtk_widget_set_size_request(panel->window, panel->geometry.width, -1);
	else
		gtk_widget_set_size_request(panel->window, -1, panel->geometry.height);
	g_signal_connect(panel->window, "size-allocate",
			G_CALLBACK(reposition_panel), panel);

	number_of_windows++;
	gdk_window_lower(rox_widget_get_window(panel->window));
...
void panel_mark_used(cairo_region_t *used)
{
	int i;
	...
	for (i = 0; i < PANEL_NUMBER_OF_SIDES; i++)
	{
		...
		gdk_window_get_root_origin(rox_widget_get_window(panel->window),
					   &rect.x, &rect.y);
		rect.width = rox_widget_get_width(panel->window);
		rect.height = rox_widget_get_height(panel->window);

		cairo_region_union_rectangle(used, &rect);
	}
}
...
void panel_update_size(void)
{
	...
	for (i = 0; i < PANEL_NUMBER_OF_SIDES; i++)
	{
		if (current_panel[i])
		{
			...
			gtk_widget_get_allocation(current_panel[i]->window, &allocation);
			reposition_panel(current_panel[i]->window, &allocation,
					current_panel[i]);
				...
				Icon *icon)
{
	gtk_widget_unset_state_flags(widget,
		GTK_STATE_FLAG_SELECTED | GTK_STATE_FLAG_PRELIGHT);
	gtk_widget_set_state_flags(widget,
		icon->selected ? GTK_STATE_FLAG_SELECTED : GTK_STATE_FLAG_PRELIGHT,
		FALSE);
...
static void panel_icon_set_tip(PanelIcon *pi)
{
	...
	else if ((!panel_want_show_text(pi)) && !pi->socket)
	{
		if (icon->item->leafname && icon->item->leafname[0])
			gtk_widget_set_tooltip_text(pi->widget, icon->item->leafname);
	}
	else
			   ...
			   gboolean locked)
{
	...
	if (!pi->socket)
	{
		...
		/* Modificado por josejp2424: el icono del panel ahora es una
		 * composición GTK3 nativa de GtkImage + GtkLabel. */
		pi->content_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
		gtk_container_add(GTK_CONTAINER(pi->widget), pi->content_box);

		pi->image_widget = gtk_image_new();
		gtk_box_pack_start(GTK_BOX(pi->content_box), pi->image_widget,
				   FALSE, FALSE, 0);

		pi->label = gtk_label_new(icon->item->leafname);
		gtk_widget_set_halign(pi->label, GTK_ALIGN_CENTER);
		gtk_widget_set_valign(pi->label, GTK_ALIGN_END);
		gtk_widget_set_margin_start(pi->label, 1);
		gtk_widget_set_margin_end(pi->label, 1);
		gtk_widget_set_margin_top(pi->label, 2);
		gtk_widget_set_margin_bottom(pi->label, 2);
		gtk_box_pack_start(GTK_BOX(pi->content_box), pi->label,
				   FALSE, FALSE, 0);
	}
...
/* Rebuild the scaled image and layout using only GTK3 public widgets. */
static void panel_icon_refresh_visual(PanelIcon *pi)
{
	Icon *icon = (Icon *) pi;
	GtkRequisition label_size = {0, 0};
	gboolean horizontal_panel;
	gboolean show_text;
	int max_width = 100;
	int max_height = 100;
	GdkPixbuf *scaled;
	...
	g_return_if_fail(pi != NULL);
	g_return_if_fail(pi->image_widget != NULL);
	g_return_if_fail(icon->item != NULL);

	horizontal_panel = pi->panel->side == PANEL_TOP ||
			     pi->panel->side == PANEL_BOTTOM;
	show_text = panel_want_show_text(pi);

	if (show_text && pi->label)
		gtk_widget_get_preferred_size(pi->label, NULL, &label_size);

	if (horizontal_panel)
		max_height = MAX(20, pi->panel->width - label_size.height);
	else
		max_width = MAX(pi->panel->width, label_size.width);

	scaled = scale_pixbuf(di_image(icon->item)->src_pixbuf,
			      MAX(20, max_width), MAX(20, max_height));
	g_clear_object(&pi->image);
	pi->image = scaled;
	gtk_image_set_from_pixbuf(GTK_IMAGE(pi->image_widget), pi->image);

	if (show_text && horizontal_panel && label_size.height > max_height)
	{
		pi->style = TEXT_BESIDE_ICON;
		gtk_orientable_set_orientation(GTK_ORIENTABLE(pi->content_box),
					       GTK_ORIENTATION_HORIZONTAL);
		gtk_widget_set_halign(pi->label, GTK_ALIGN_END);
		gtk_widget_set_valign(pi->label, GTK_ALIGN_CENTER);
	}
	else
	{
	...
	gtk_box_set_spacing(GTK_BOX(pi->content_box),
			    horizontal_panel ? PANEL_ICON_SPACING / 2 : 0);
	gtk_widget_set_visible(pi->label, show_text);
...
static void panel_setup_struts(Panel *panel, GtkAllocation *alloc)
{
	...
	if (thickness)
	{
		...
		gdk_property_change(rox_widget_get_window(panel->window),
	...
	{
		gdk_property_delete(rox_widget_get_window(panel->window),
				gdk_atom_intern("_NET_WM_STRUT_PARTIAL",
					FALSE));
		gdk_property_delete(rox_widget_get_window(panel->window),
				gdk_atom_intern("_NET_WM_STRUT",
				...
				GtkAllocation *alloc, Panel *panel)
{
	...
	gdk_window_move(rox_widget_get_window(panel->window), x, y);
			    ...
			    PanelIcon		*pi)
{
	...
	rox_gdk_window_get_pointer(rox_widget_get_window(pi->panel->window),
			 ...
			 Panel		    *panel)
{
	...
	{
		const guchar *sd_data = gtk_selection_data_get_data(selection_data);
		gint sd_len = gtk_selection_data_get_length(selection_data);
		char *sd_text = NULL;

		if (!sd_data || sd_len <= 0)
			return;

		sd_text = g_strndup((const char *) sd_data, (gsize) sd_len);
		uris = uri_list_to_glist(sd_text);
		g_free(sd_text);
	}
			      ...
			      Panel *panel)
{
	...
	if (event->x >= 0 &&
	    event->y >= 0 &&
	    event->x < rox_widget_get_width(widget) &&
	    event->y < rox_widget_get_height(widget))
	{
...
static void motion_may_raise(Panel *panel, int x, int y)
{
	...
	if (panel->side == PANEL_TOP)
		...
		raise = y == rox_widget_get_height(panel->window) - 1;
	else if (panel->side == PANEL_LEFT)
		raise = x == 0;
	else
		raise = x == rox_widget_get_width(panel->window) - 1;

	if (raise && panel_keep_below(panel, FALSE))
	{
		/* Shouldn't need this as well as keep_below but some WMs don't
		 * automatically raise as soon as the hint is set */
		gdk_window_raise(rox_widget_get_window(panel->window));
	}
}
...
static gboolean may_autoscroll(Panel *panel)
{
	...
	if (gtk_adjustment_get_upper(panel->adj) <= gtk_adjustment_get_page_size(panel->adj))
		goto stop_scrolling;	/* Can see everything already */

	rox_gdk_window_get_pointer(rox_widget_get_window(panel->window), &panel_x, &panel_y, NULL);

	if (horz)
	{
		delta = panel_x;
		max   = rox_widget_get_width(panel->window);
		if (panel_y < 0 || panel_y > rox_widget_get_height(panel->window))
			goto stop_scrolling;	/* Not over the panel */
	}
	else
	{
		delta = panel_y;
		max   = rox_widget_get_height(panel->window);
		if (panel_x < 0 || panel_x > rox_widget_get_width(panel->window))
			goto stop_scrolling;	/* Not over the panel */
	}
	...
	new = gtk_adjustment_get_value(panel->adj) - ((delta < 20) ? panel->autoscroll_speed
						: -panel->autoscroll_speed);
	new = CLAMP(new, 0, gtk_adjustment_get_upper(panel->adj) - gtk_adjustment_get_page_size(panel->adj));
			      ...
			      PanelIcon *pi)
{
	...
	if (me->prev)
	{
		...
		gdk_window_get_origin(rox_widget_get_window(prev), &x, &y);
	...
	if (dir == 0 && me->next)
	{
		...
		gdk_window_get_origin(rox_widget_get_window(next), &x, &y);

		w = gtk_widget_get_allocated_width(next);
		h = gtk_widget_get_allocated_height(next);
				    ...
				    int index)
{
	...
	if (!g_list_find(list, widget))
	{
		...
		/* Move between panel boxes through public GTK3 container APIs. */
		g_object_ref(widget);
		if (parent && GTK_IS_CONTAINER(parent))
			gtk_container_remove(GTK_CONTAINER(parent), widget);
		gtk_box_pack_start(GTK_BOX(side), widget, FALSE, FALSE, 0);
...
static void applet_died(GtkWidget *socket)
{
	...
	never_plugged = (!g_object_get_data(G_OBJECT(socket), "lost_plug"))
		      && gtk_socket_get_plug_window(GTK_SOCKET(socket)) == NULL;
...
static void run_applet(PanelIcon *pi)
{
	...
	XGrabButton(rox_x11_display(), Button2, AnyModifier,
			gdk_x11_window_get_xid(rox_widget_get_window(pi->socket)),
			False,
...

	{
		...
		gdk_property_change(rox_widget_get_window(pi->socket),
				...
				Panel *panel)
{
	...
	if (panel->side == PANEL_TOP || panel->side == PANEL_BOTTOM)
	{
		...
		height = EDGE_WIDTH;
		x = 0;
		y = panel->side == PANEL_BOTTOM ? 0 :
			gtk_widget_get_allocated_height(widget) - EDGE_WIDTH;
	}
	else
	{
		width = EDGE_WIDTH;
		...
		y = 0;
		x = panel->side == PANEL_RIGHT ? 0 :
			gtk_widget_get_allocated_width(widget) - EDGE_WIDTH;
	}

	/* Modificado por josejp2424: borde del panel dibujado con Cairo. */
	context = gtk_widget_get_style_context(widget);
	gtk_style_context_get_color(context,
		gtk_style_context_get_state(context), &colour);
	gdk_cairo_set_source_rgba(cr, &colour);
	cairo_rectangle(cr, x, y, MAX(0, width), MAX(0, height));
	cairo_fill(cr);
	return FALSE;
}
...
static void panel_icon_redraw(Icon *icon)
{
	...
	if (icon->selected)
		gtk_widget_set_state_flags(pi->widget, GTK_STATE_FLAG_SELECTED, FALSE);
	else
		gtk_widget_unset_state_flags(pi->widget, GTK_STATE_FLAG_SELECTED);
...
static void panel_xinerama_changed(Panel *panel)
{
	...
	panel_update_geometry(panel);
	gtk_widget_get_allocation(panel->window, &allocation);
	reposition_panel(panel->window, &allocation, panel);
	gtk_widget_queue_resize(panel->window);
	panel_save(panel);
}
...
static void panel_avoid_toggled_cb(GtkToggleButton *widget)
{
	...
	if (avoid != panel->avoid)
	{
		...
		panel->avoid = avoid;
		gtk_widget_get_allocation(panel->window, &allocation);
		panel_setup_struts(panel, &allocation);
		panel_save(panel);
	}
}
...
static void panel_show_menu(GdkEventButton *event, PanelIcon *pi, Panel *panel)
{
	...
	option_item = gtk_menu_item_new_with_label(_("Panel Options..."));
	g_signal_connect_swapped(option_item, "activate",
			 G_CALLBACK(panel_show_options), panel);

	del_item = gtk_menu_item_new_with_label(_("Remove Panel"));
	menu_item_set_icon(del_item, ROX_ICON_REMOVE);
		       ...
		       Panel		*panel)
{
	...
	window = rox_widget_get_window(panel->window);
	gtk_widget_get_allocation(panel->window, &allocation);
	rox_gdk_window_get_pointer(window, &x, &y, NULL);
...
static GList *build_monitor_number(Option *option, xmlNode *node, guchar *label)
{
	...
	return build_numentry_base(option, node, label, adj);
}
...
static gboolean panel_keep_below(Panel *panel, gboolean setting)
{
	...
	{
		keep_below(rox_widget_get_window(panel->window), setting);
		return TRUE;
	}
	return FALSE;
}
