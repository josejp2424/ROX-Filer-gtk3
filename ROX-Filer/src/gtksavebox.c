...
/* Compat: en GTK3 GdkDragContext es opaco (no se accede a ->source_window). */
#if GTK_MAJOR_VERSION >= 3
#define ROX_DND_SOURCE_WINDOW(ctx) gdk_drag_context_get_source_window((ctx))
#else
#define ROX_DND_SOURCE_WINDOW(ctx) ((ctx)->source_window)
#endif
...
gtk_savebox_init (GtkSavebox *savebox)
{
  ...
  g_signal_connect (savebox, "drag_data_get",
	...
	gtk_widget_set_halign (savebox->drag_box, GTK_ALIGN_CENTER);
	gtk_widget_set_valign (savebox->drag_box, GTK_ALIGN_CENTER);
	gtk_box_pack_start (GTK_BOX (gtk_dialog_get_content_area (GTK_DIALOG (dialog))), savebox->drag_box, TRUE, TRUE, 0);
  savebox->entry = gtk_entry_new ();
  g_signal_connect_swapped (savebox->entry, "activate",
			     G_CALLBACK (do_save), savebox);
  gtk_box_pack_start (GTK_BOX (gtk_dialog_get_content_area(GTK_DIALOG(dialog))), savebox->entry, FALSE, TRUE, 4);

  gtk_widget_show_all (gtk_dialog_get_content_area(GTK_DIALOG(dialog)));
  gtk_widget_grab_focus (savebox->entry);

  savebox->discard_area = gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);

  button = button_new_mixed (ROX_ICON_DELETE, "_Discard");
  ...
  gtk_widget_set_can_focus(button, FALSE); /* Modificado por josejp2424: GTK3 */
  gtk_widget_set_can_default(button, TRUE); /* Modificado por josejp2424: GTK3 */

  gtk_box_pack_end (GTK_BOX (gtk_dialog_get_content_area(GTK_DIALOG(dialog))), savebox->discard_area,
		      FALSE, TRUE, 0);
  gtk_box_reorder_child (GTK_BOX (gtk_dialog_get_content_area(GTK_DIALOG(dialog))), savebox->discard_area, 0);
...
gtk_savebox_new (const gchar *action)
{
  ...
  {
    GtkWidget *cancel = dialog_add_icon_button(dialog, ROX_ICON_CANCEL,
                                               _("_Cancel"), GTK_RESPONSE_CANCEL);
    gtk_widget_set_can_focus(cancel, FALSE);
  }

  button = button_new_mixed (ROX_ICON_SAVE, action);
  gtk_widget_set_can_default(button, TRUE); /* Modificado por josejp2424: GTK3 */
			...
			GtkSavebox *savebox)
{
  ...
  context = gtk_drag_begin_with_coordinates (GTK_WIDGET (savebox),
			    savebox->targets, savebox->dnd_action,
			    event->button, (GdkEvent *) event, -1, -1);
               ...
               guint32          time)
{
  ...
  gtk_selection_data_set (selection_data, xa_string, 8,
			  (const guchar *) &to_send, 1);
}
...
read_xds_property (GdkDragContext *context, gboolean delete)
{
  ...
  if (gdk_property_get (ROX_DND_SOURCE_WINDOW(context), XdndDirectSave, text_plain,
...
write_xds_property (GdkDragContext *context, const guchar *value)
{
  ...
  if (value)
    {
      gdk_property_change (ROX_DND_SOURCE_WINDOW(context), XdndDirectSave,
			   text_plain, 8, GDK_PROP_MODE_REPLACE,
			   value, strlen (value));
    }
  else
    gdk_property_delete (ROX_DND_SOURCE_WINDOW(context), XdndDirectSave);
}
