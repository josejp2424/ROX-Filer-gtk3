...
/* Label support */
GdkRGBA *xlabel_get(const char *path)
{
	GdkRGBA *col = NULL;
	gchar *buf;
	char *nl;
	...
	if(buf)
	{
		...
		if(nl)
			*nl = 0;
		col = g_new(GdkRGBA, 1);
		if (gdk_rgba_parse(col, buf) == FALSE) {
			g_free(col);
			col = NULL;
		}
		g_free(buf);
...
void xattrs_browser(DirItem *item, const guchar *path)
{
	...
	dialog_add_icon_button(dialog, ROX_ICON_APPLY, _("_Apply"), GTK_RESPONSE_APPLY);
	dialog_add_icon_button(dialog, ROX_ICON_CLOSE, _("_Close"), GTK_RESPONSE_CLOSE);
