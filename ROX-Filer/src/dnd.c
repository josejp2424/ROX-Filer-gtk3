...
/* GTK3 public accessors for DND state and selection payloads.
 * Modificado por josejp2424 */
#define DND_CONTEXT_SOURCE_WINDOW(ctx) (gdk_drag_context_get_source_window((ctx)))
#define DND_CONTEXT_TARGETS(ctx) ((GList *) gdk_drag_context_list_targets((ctx)))
#define DND_CONTEXT_ACTIONS(ctx) (gdk_drag_context_get_actions((ctx)))
#define DND_CONTEXT_SUGGESTED_ACTION(ctx) (gdk_drag_context_get_suggested_action((ctx)))
#define DND_CONTEXT_ACTION(ctx) (gdk_drag_context_get_selected_action((ctx)))
#define DND_CONTEXT_PROTOCOL(ctx) (gdk_drag_context_get_protocol((ctx)))

#define DND_SELECTION_GET_DATA(sd) ((guchar *) gtk_selection_data_get_data((sd)))
#define DND_SELECTION_GET_LENGTH(sd) (gtk_selection_data_get_length((sd)))
#define DND_SELECTION_GET_FORMAT(sd) (gtk_selection_data_get_format((sd)))
#define DND_SELECTION_GET_TARGET(sd) (gtk_selection_data_get_target((sd)))
#define DND_SELECTION_GET_TYPE(sd) (gtk_selection_data_get_data_type((sd)))

#define DND_WIDGET_WINDOW(w) (gtk_widget_get_window((w)))
 ...
 * GTK3 already supplies an implicit pointer grab for a button press.  We rely
 * on that normal widget-level grab here; an explicit GdkSeat grab is only
 * reacquired by dnd_motion_grab_pointer() when panel reparenting requires it.
 *
...
static void set_xds_prop(GdkDragContext *context, const char *text)
{
	gdk_property_change(DND_CONTEXT_SOURCE_WINDOW(context),
			XdndDirectSave0,
			xa_text_plain, 8,
...
static char *get_xds_prop(GdkDragContext *context)
{
	...
	if (gdk_property_get(DND_CONTEXT_SOURCE_WINDOW(context),
			XdndDirectSave0,
			xa_text_plain,
			0, MAXURILEN,
			FALSE,
...
{
	GList	    *targets = DND_CONTEXT_TARGETS(context);
...
void drag_selection(GtkWidget *widget, GdkEventMotion *event, guchar *uri_list)
{
	...
	/* Modificado por josejp2424: icono DND tomado del tema GTK3 activo. */
	gtk_drag_set_icon_name(context,
				rox_icon_name(ROX_ICON_DND_MULTIPLE), 0, 0);
}
			...
			gpointer		data)
{
	...
	type = DND_SELECTION_GET_TARGET(selection_data);
			...
			guint32        		time)
{
	...
	utf8 = g_utf16_to_utf8((gunichar2 *) DND_SELECTION_GET_DATA(selection_data),
			(glong) DND_SELECTION_GET_LENGTH(selection_data),
			NULL, NULL, NULL);
			       ...
			       gpointer		user_data)
{
	...
	switch (info)
	{
		...
		case TARGET_RAW:
			...
			break;
		case TARGET_URI_LIST:
			got_uri_list(widget, context, DND_SELECTION_GET_DATA(selection_data),
					time);
			break;
		case TARGET_MOZ_URL:
				...
				guint32             	time)
{
	...
	/* Modificado por josejp2424 */
	guchar *sd_data = DND_SELECTION_GET_DATA(selection_data); /* Modificado por josejp2424 */
	int sd_len = DND_SELECTION_GET_LENGTH(selection_data); /* Modificado por josejp2424 */
	char response = (sd_data && sd_len > 0) ? (char) sd_data[0] : 0; /* Modificado por josejp2424 */
	const char	*error = NULL;
	char		*dest_path;
			...
			guint32             	time)
{
	...
	g_return_if_fail(DND_SELECTION_GET_DATA(selection_data) != NULL);

	dest_path = g_dataset_get_data(context, "drop_dest_path");

	if (DND_CONTEXT_ACTION(context) == GDK_ACTION_ASK)
	{
		gtk_drag_finish(context, FALSE, FALSE, time);	/* Failure */
	...
	if (g_dataset_get_data(context, "drop_dest_type") == drop_dest_prog)
	{
		...
		run_with_data(dest_path,
				DND_SELECTION_GET_DATA(selection_data), DND_SELECTION_GET_LENGTH(selection_data));
	...
	{
		if (write(fd,
			DND_SELECTION_GET_DATA(selection_data),
			DND_SELECTION_GET_LENGTH(selection_data)) == -1)
				error = g_strerror(errno);
			 ...
			 guint32             	time)
{
	...
	else if (DND_CONTEXT_ACTION(context) != GDK_ACTION_ASK && type == drop_dest_prog)
	...
	else
	{
		...
		else if (DND_CONTEXT_ACTION(context) == GDK_ACTION_LINK)
...
static void prompt_action(GList *paths, gchar *dest)
{
	...
	if (!dnd_menu)
	{
		...
		item_factory = menu_create(menu_def,
				...
				"<dnd>", NULL);
		dnd_menu = rox_item_factory_get_widget(item_factory, "<dnd>");
	}
...
void dnd_spring_load(GdkDragContext *context, FilerWindow *src_win)
{
	...
	spring_timeout = g_timeout_add( /* Modificado por josejp2424 */
...
void dnd_spring_abort(void)
{
	...
	spring_context = NULL;
	g_source_remove( /* Modificado por josejp2424 */spring_timeout);
}
...
static gboolean spring_now(gpointer data)
{
	...
	else
	{
		...
		if (spring_window)
		{
			g_timeout_add( /* Modificado por josejp2424 */500, spring_check_idle, NULL);
			g_signal_connect(spring_window->window, "destroy",
					G_CALLBACK(spring_win_destroyed), NULL);
			centre_window(DND_WIDGET_WINDOW(spring_window->window), x, y);
		}
	}
	spring_in_progress--;
...
/* Active GTK3 seat grab used while an icon is being repositioned. */
static GdkSeat *motion_grab_seat = NULL;

static gboolean motion_grab_pointer(GtkWidget *widget,
                                      ...
                                      GdkEvent *trigger_event)
{
	...
	/* No-window widgets such as Collection share a parent GdkWindow.
	 * Use the window that actually delivered the button event whenever
	 * possible, otherwise a seat grab can redirect the release to the
	 * viewport instead of back to the file view.
	 */
	window = event_window ? event_window : gtk_widget_get_window(widget);
	if (!window)
		return FALSE;
	...
	status = gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_POINTER,
		TRUE, NULL, trigger_event, NULL, NULL);
	if (status != GDK_GRAB_SUCCESS)
		return FALSE;

	motion_grab_seat = seat;
	return TRUE;
}
...
{
	GdkModifierType held_buttons;

	if (event->type != GDK_BUTTON_PRESS)
		return TRUE;		/* Not a click event! */

	held_buttons = event->state &
		(GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK);

	/* A lost release must not permanently disable clicks and menus.  On a
	 * button-press event event->state describes buttons held before this new
	 * press.  If our counter says a button is down but GDK says none is held,
	 * the old state is stale and can be safely reset.
	 */
	if (motion_buttons_pressed > 0 && held_buttons == 0)
	{
		INPUT_TRACE("recover stale state before press: count=%d motion=%d",
			motion_buttons_pressed, motion_state);
		dnd_motion_ungrab();
	}

	INPUT_TRACE("press type=%d button=%u state=0x%x count=%d motion=%d widget=%s window=%p",
		event->type, event->button, (guint) event->state,
		motion_buttons_pressed, motion_state,
		G_OBJECT_TYPE_NAME(widget), (void *) event->window);

	motion_buttons_pressed++;
	if (motion_buttons_pressed == 1)
	{
		motion_widget = widget;
		/* Do not call gdk_seat_grab() here.  GTK3's implicit grab keeps
		 * press/motion/release routed to the originating widget and works
		 * correctly for Collection, which is a no-window widget.
		 */
	}

	if (motion_state != MOTION_NONE)
	{
		INPUT_TRACE("press ignored: busy count=%d motion=%d",
			motion_buttons_pressed, motion_state);
...
gboolean dnd_motion_release(GdkEventButton *event)
{
	...
	INPUT_TRACE("release type=%d button=%u state=0x%x count=%d motion=%d window=%p",
		event->type, event->button, (guint) event->state,
		motion_buttons_pressed, motion_state, (void *) event->window);

	if (motion_buttons_pressed == 0)
	{
		INPUT_TRACE("release ignored: motion system already reset");
	...
	INPUT_TRACE("release click decision: dx=%d dy=%d threshold=%d eat=%d",
		dx, dy, drag_threshold,
...
{
	INPUT_TRACE("ungrab count=%d motion=%d explicit_seat=%p",
		motion_buttons_pressed, motion_state, (void *) motion_grab_seat);
	if (motion_buttons_pressed > 0)
	{
		if (motion_grab_seat)
		{
			gdk_seat_ungrab(motion_grab_seat);
			motion_grab_seat = NULL;
		}
		motion_widget = NULL;
...
void dnd_motion_grab_pointer(void)
{
	...
	if (motion_grab_seat)
		gdk_seat_ungrab(motion_grab_seat);
	motion_grab_seat = NULL;
	motion_grab_pointer(motion_widget, NULL, NULL);
}
