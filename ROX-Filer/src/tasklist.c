...
static GArray *get_window_list(Window xwindow, GdkAtom atom)
{
	...
	type = None;
	result = XGetWindowProperty(rox_x11_display(),
			xwindow,
			gdk_x11_atom_to_xatom(atom),
			0, G_MAXLONG,
			False, XA_WINDOW, &type, &format, &nitems,
			&bytes_after, &data);
	err = rox_x11_error_trap_pop();
...
static guchar *get_str(IconWindow *win, GdkAtom atom)
{
	...
	result = XGetWindowProperty(rox_x11_display(), win->xwindow,
			gdk_x11_atom_to_xatom(atom),
			0, G_MAXLONG, False,
			AnyPropertyType,
			&rtype, &format, &nitems,
...
static gboolean within_viewport(Window xwindow)
{
  ...
  XGetGeometry (rox_x11_display(),
                xwindow,
                &root_window,
                &x, &y, &width, &height, &bw, &depth);
  XTranslateCoordinates (rox_x11_display(),
                         xwindow,
			 root_window,
                         0, 0,
                         &x, &y, &child);
...
static void window_check_status(IconWindow *win)
{
	...
	if (XGetTransientForHint(rox_x11_display(), win->xwindow, &transient_for) && transient_for)
		iconic = FALSE;
	else if (wm_supports_hidden && XGetWindowProperty(rox_x11_display(), win->xwindow,
	...
	else if (XGetWindowProperty(rox_x11_display(), win->xwindow,
	...
	if (o_pinboard_tasklist_per_workspace.int_value &&
	    XGetWindowProperty(rox_x11_display(), win->xwindow,
...
static void update_current_desktop(void)
{
	...
	if (XGetWindowProperty(rox_x11_display(), gdk_x11_get_default_root_xwindow(),
	...
	if (xev->type == PropertyNotify)
	{
		...
		if (atom == xa_WM_STATE || atom == xa__NET_WM_STATE)
		{
			...
			if (w)
			{
				...
				if (rox_x11_error_trap_pop() != Success)
...
static void add_window(Window win)
{
	...
	if (!w)
	{
		...
		XGetWindowAttributes(rox_x11_display(), win, &attr);

		if (rox_x11_error_trap_pop() != Success)
			return;
		rox_x11_error_trap_push();

		XSelectInput(rox_x11_display(), win, attr.your_event_mask |
			PropertyChangeMask);
...
static void uniconify(IconWindow *win, guint32 timestamp)
{
	...
	XSendEvent(rox_x11_display(), DefaultRootWindow(rox_x11_display()), False,
			SubstructureNotifyMask | SubstructureRedirectMask,
			(XEvent *) &sev);
	XSync(rox_x11_display(), False);

	rox_x11_error_trap_pop();
}
			       ...
			       IconWindow *win)
{
	if (event->state & GDK_BUTTON1_MASK)
	{
		...
		if (!drag_started)
		{
			...
			GtkWidget *parent = gtk_widget_get_parent(win->widget);
			if (parent)
				fixed_move_fast(GTK_FIXED(parent),
						win->widget,
 ...
 *
 * XWMHints exposes its icon as a server-side X11 Pixmap.  This GTK3 path
 * deliberately avoids private GDK conversion helpers; when no themed icon is
 * available, get_image_for() falls back to ROX-Filer's bundled default icon.
 */
...
static GdkPixbuf *get_image_for(IconWindow *win)
{
	...
	rox_x11_error_trap_push();
	hints = XGetWMHints(rox_x11_display(), win->xwindow);
...
static void show_icon(IconWindow *win)
{
	...
	gtk_widget_set_can_focus(win->widget, FALSE);
	gtk_style_context_add_class(gtk_widget_get_style_context(win->widget),
		"rox-pin-icon");
	...
	gtk_style_context_add_class(gtk_widget_get_style_context(win->label),
		"rox-pin-label");
...
static void set_iconify_pos(IconWindow *win)
{
	...
	XChangeProperty(rox_x11_display(), win->xwindow,
