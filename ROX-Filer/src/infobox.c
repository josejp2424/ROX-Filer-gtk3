...
static void add_file_output(FileStatus *fs,
			    gint source, RoxInputCondition condition);
...
void infobox_show_list(GList *paths)
{
	...
	if (n >= 10)
	{
		...
		ok = confirm(message, ROX_ICON_YES, _("Show Info"));
		g_free(message);
		if (!ok)
			return;
	}
...
void infobox_new(const gchar *pathname)
{
	...
	/* GTK_DIALOG_NO_SEPARATOR was removed in GTK3 (dialogs no longer draw the
	 * old separator). Keep flags = 0 here.
	 */
	window = gtk_dialog_new();
	gtk_window_set_title(GTK_WINDOW(window),
			g_utf8_validate(path, -1, NULL) ? path : _("(bad utf-8)"));
	dialog_add_icon_button(GTK_DIALOG(window), ROX_ICON_CLOSE,
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(window))), details, TRUE, TRUE, 0);
...
static void refresh_info(GObject *window)
{
	...
	/* GtkWidget is opaque in GTK3; use accessors instead of struct fields. */
...
static GtkWidget *make_vbox(const guchar *path, GObject *window)
{
	...
	if (access(help_dir, F_OK) == 0)
	{
		...
		button = button_new_mixed(ROX_ICON_JUMP_TO,
				_("Show _Help Files"));
...
static GtkWidget *make_file_says(const guchar *path)
{
	...
	switch (fork())
	{
		...
		default:
			...
			fs->fd = file_data[0];
			fs->text = g_strdup("");
			fs->input = rox_input_add_full(fs->fd, ROX_INPUT_READ,
				(RoxInputFunction) add_file_output,
				fs, NULL);
...
static GtkWidget *make_permissions(const gchar *path, DirItem *item)
{
	...
	/* Modificado por josejp2424: matriz de permisos nativa con GtkGrid. */
	table = gtk_grid_new();
	...
	for (i = 0; i < 9; i++)
	{
		...
		gtk_grid_attach(GTK_GRID(table), tick, x, y, 1, 1);
		if (item->mode & (1 << i))
	...
	gtk_grid_attach(GTK_GRID(table), tick, 4, 1, 1, 1);
	...
	gtk_grid_attach(GTK_GRID(table), tick, 4, 2, 1, 1);
	...
	gtk_grid_attach(GTK_GRID(table), tick, 4, 3, 1, 1);
