...
{N_("Remove Item(s)"),		NULL, remove_items, 0, "<IconItem>", ROX_ICON_REMOVE},
{"",				NULL, NULL, 0, "<Separator>"},
};
...
void icon_prepare_menu(Icon *icon, GtkWidget *options_item, ...)
{
	...
	menu_item_set_icon(options_item, ROX_ICON_PREFERENCES);
...
void icon_set_path(Icon *icon, const char *pathname, const char *name)
{
	...
	if (pathname)
	{
		...
		if (icon->item->mime_type == application_x_desktop && name_set == FALSE)
		{
				      ...
				      gpointer data)
{
	...
	if (gtk_widget_get_window(popup))
		gdk_window_remove_filter(gtk_widget_get_window(popup), filter_get_key, data);
...
static void get_shortcut(GtkWidget *button, GtkWidget *label)
{
	...
	rox_widget_set_padding(GTK_WIDGET(msg), 20, 20);
	...
	pwin = gtk_widget_get_window(popup);
	if (!pwin)
	{
		delayed_error(_("Popup window not realized!"));
		gtk_widget_destroy(popup);
		return;
	}
	gdk_window_add_filter(pwin, filter_get_key, popup);

	xid = gdk_x11_window_get_xid(pwin);
...
static void show_rename_box(Icon *icon)
{
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), (GtkWidget *) vbox,
			   TRUE, TRUE, 0);
	...
	label = gtk_label_new(icon->shortcut ? icon->shortcut : CLICK_TO_SET);
	button = gtk_button_new();
	gtk_container_add(GTK_CONTAINER(button), label);
	gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
	g_object_set_data(G_OBJECT(dialog), "new_shortcut", label);
	g_signal_connect(button, "clicked", G_CALLBACK(get_shortcut), label);
	button2 = button_new_icon(ROX_ICON_CLEAR);
	gtk_box_pack_start(GTK_BOX(hbox), button2, FALSE, FALSE, 0);
	g_signal_connect(button2, "clicked", G_CALLBACK(clear_shortcut), label);
	...
	gtk_widget_set_tooltip_text(lock_state, _("Locking an item prevents it from being accidentally removed"));
	...
	dialog_add_icon_button(dialog, ROX_ICON_OK, _("_OK"), GTK_RESPONSE_OK);
...
static gboolean grabKey(MyKey *key)
{
	...
	rox_display_flush();
	return rox_x11_error_trap_pop() == Success;
}
...
static void create_menu(void)
{
	...
	icon_menu = rox_item_factory_get_widget(item_factory, "<icon>");
	icon_file_menu = rox_item_factory_get_widget(item_factory, tmp);
	g_free(tmp);

	/* File '' label... */
	items = gtk_container_get_children(GTK_CONTAINER(icon_menu));
	icon_file_item = menu_item_get_label_widget(GTK_WIDGET(g_list_nth(items, 1)->data));
	g_list_free(items);

	/* Shift Open... label */
	items = gtk_container_get_children(GTK_CONTAINER(icon_file_menu));
	file_shift_item = menu_item_get_label_widget(GTK_WIDGET(items->data));
	g_list_free(items);
