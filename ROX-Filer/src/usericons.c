...
static void clear_icon(DropBox *drop_box, GObject *dialog)
{
	...
	else
	{
		...
		if (confirm(tmp, ROX_ICON_DELETE, NULL))
		{
			if (unlink(path))
...
void icon_set_handler_dialog(DirItem *item, const guchar *path)
{
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), frame, TRUE, TRUE, 4);
	...
	dialog_add_icon_button(dialog, ROX_ICON_CLOSE, _("_Close"), GTK_RESPONSE_OK);
