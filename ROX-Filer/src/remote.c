...
gboolean remote_init(xmlDocPtr rpc, gboolean new_copy)
{
	...
	XGrabServer(gdk_x11_display_get_xdisplay(gdk_display_get_default()));
	...
	if (existing_ipc_window)
	{
		...
		XUngrabServer(gdk_x11_display_get_xdisplay(gdk_display_get_default()));
	...
	/* ClientMessage events are X11-specific and are no longer exposed as
	 * a generic GDK event in GTK3. Attach a native X11 event filter to the
	 * realised IPC window instead.
	 */
	gdk_window_add_filter(gtk_widget_get_window(ipc_window),
			client_event_filter, NULL);
...
static GdkWindow *get_existing_ipc_window(void)
{
	...
	if (gdk_x11_window_lookup_for_display(gdk_display_get_default(), xid))
					 ...
					 gpointer user_data)
{
	...
	if (!native_event || native_event->type != ClientMessage)
		return GDK_FILTER_CONTINUE;

	message = &native_event->xclient;
	if (message->message_type != gdk_x11_atom_to_xatom(xsoap) ||
	    message->format != 32)
		return GDK_FILTER_CONTINUE;

	src_window = gdk_x11_window_foreign_new_for_display(
			gdk_display_get_default(), (Window) message->data.l[0]);
	if (!src_window)
	{
	...
	prop = gdk_x11_xatom_to_atom((Atom) message->data.l[1]);
	data = read_property(src_window, prop, &length);
	if (!data)
	{
		g_object_unref(src_window);
		return GDK_FILTER_REMOVE;
	}

	doc = xmlParseMemory((const char *) data, length);
	g_free(data);
	if (!doc)
	{
		g_warning("Invalid SOAP message received");
		g_object_unref(src_window);
		return GDK_FILTER_REMOVE;
	}
...
static void soap_send(GtkWidget *from, GdkAtom prop, GdkWindow *dest)
{
	...
	xevent.xclient.message_type = gdk_x11_atom_to_xatom(xsoap);
	xevent.xclient.format = 32;
	xevent.xclient.data.l[0] =
		gdk_x11_window_get_xid(gtk_widget_get_window(from));
	xevent.xclient.data.l[1] = gdk_x11_atom_to_xatom(prop);
