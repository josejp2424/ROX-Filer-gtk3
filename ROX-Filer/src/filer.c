...
void filer_init(void)
{
	...
	busy_cursor = gdk_cursor_new_for_display(gdk_display_get_default(), GDK_WATCH);
	 ...
	 */
	ohost = our_host_name();
	dpy = gdk_display_get_name(gdk_display_get_default());
...
/* Default size used only when a directory has no saved geometry.
 * This prevents GTK from opening a newly-created filer window as a very
 * wide, shallow strip based on the current icon requisition.  It is not a
...
void filer_window_set_size(FilerWindow *filer_window, int w, int h)
{
	...
	if (GTK_IS_WIDGET(filer_window->scrollbar))
	{
		gtk_widget_get_allocation(filer_window->scrollbar, &alloc);
		w += alloc.width;
	}

	if (GTK_IS_WIDGET(filer_window->message))
	{
		gtk_widget_get_allocation(filer_window->message, &alloc);
		h += alloc.height;
	}

	if (GTK_IS_WIDGET(filer_window->toolbar))
	{
	...
	/* Query window geometry through public GTK3 accessors. */
	gtk_widget_get_preferred_size(window, &min_req, NULL);
	w = MAX(w, min_req.width);
	h = MAX(h, min_req.height);

	gdk_window = gtk_widget_get_window(window);
	if (!gdk_window)
		return;

	/* In GTK3 we don't keep window_x/window_y fields on FilerWindow.
	 * Ask the GdkWindow for the current position and clamp against the
...

	{
		...
		if (display)
		{
			mon = gdk_display_get_monitor_at_point(display, x, y);
			if (!mon)
				mon = gdk_display_get_monitor_at_window(display, gdk_window);
			if (mon)
			{
		...
		if (have_rect)
		{
			...
			if (nx + w + DECOR_BORDER > rect.x + rect.width)
				nx = rect.x + rect.width - w - 4 - DECOR_BORDER;
			if (ny + h + DECOR_BORDER > rect.y + rect.height)
				ny = rect.y + rect.height - h - 4 - DECOR_BORDER;

			nx = MAX(nx, rect.x);
			ny = MAX(ny, rect.y);

			gdk_window_move_resize(gdk_window, nx, ny, w, h);
		}
		else
	...
	if (event && event->type == GDK_KEY_PRESS)
	{
		...
		rox_gdk_window_get_pointer(gdk_window, &px, &py, NULL);

		nx = CLAMP(px, 4, w - 4);
		ny = CLAMP(py, 4, h - 4);

#ifdef GDK_WINDOWING_X11
		if ((nx != px || ny != py) && GDK_IS_X11_WINDOW(gdk_window))
		{
			Display *dpy = gdk_x11_display_get_xdisplay(
				gdk_window_get_display(gdk_window));
			Window xid = gdk_x11_window_get_xid(gdk_window);

			XWarpPointer(dpy, None, xid, 0, 0, 0, 0, nx, ny);
		}
#endif
	}
	if (event)
...
static void may_offer_unmount(FilerWindow *filer_window, char *mount)
{
	...
	button = button_new_mixed(ROX_ICON_MOUNTED, _("No change"));
	gtk_widget_set_can_default(button, TRUE); /* Modificado por josejp2424: GTK3 */
	gtk_dialog_add_action_widget(GTK_DIALOG(dialog), button,
					GTK_RESPONSE_CANCEL);
	gtk_widget_show(button);

	button = button_new_mixed(ROX_ICON_MOUNT, _("Unmount"));
	gtk_widget_set_can_default(button, TRUE); /* Modificado por josejp2424: GTK3 */
	gtk_dialog_add_action_widget(GTK_DIALOG(dialog), button,
					GTK_RESPONSE_OK);
	gtk_widget_show(button);
	...
	button = button_new_mixed(ROX_ICON_UNDO, _("Eject"));
	gtk_widget_set_can_default(button, TRUE); /* Modificado por josejp2424: GTK3 */
	gtk_dialog_add_action_widget(GTK_DIALOG(dialog), button,
					ROX_RESPONSE_EJECT);
	gtk_widget_show(button);
...
void filer_next_selected(FilerWindow *filer_window, int dir)
{
	...
	if (iter.next(&iter))
		...
		gdk_display_beep(gtk_widget_get_display(filer_window->window));
			   ...
			   const gchar *wm_class)
{
	...
	/* Keep the standard or saved initial geometry until the window has
	 * actually been shown.  The automatic icon-layout resizer must not
	 * replace it during construction.
	 */
	filer_window->initial_geometry_pending = TRUE;
	filer_window->initial_geometry_saved = FALSE;
...
void filer_set_view_type(FilerWindow *filer_window, ViewType type)
{
	...
	switch (type)
	{
		case VIEW_TYPE_COLLECTION:
			/* Modificado por josejp2424: vista de iconos GTK3 reactivada. */
...
static void filer_add_widgets(FilerWindow *filer_window, const gchar *wm_class)
{
	...
	if (wm_class)
		rox_window_set_wmclass(GTK_WINDOW(filer_window->window),
				      wm_class, PROJECT);

	/* GTK3: Colormaps were removed; use an RGBA visual instead (for transparency
	 * with compositors). If an RGBA visual is not available, GTK will fall back
	 * to the default visual automatically.
	 */
	GdkScreen *screen = gtk_widget_get_screen(filer_window->window);
	GdkVisual *visual = gdk_screen_get_rgba_visual(screen);
	if (visual)
		gtk_widget_set_visual(filer_window->window, visual);
	...
	filer_window->scrollbar = gtk_scrollbar_new(GTK_ORIENTATION_VERTICAL, NULL);
			...
			const char *reason)
{
	...
	if (fn != old_fn)
		{
				GdkWindow *view_window = gtk_widget_get_window(GTK_WIDGET(filer_window->view));
				if (view_window)
					gdk_window_set_cursor(view_window, fn ? crosshair : NULL);
			}
...
void filer_perform_action(FilerWindow *filer_window, GdkEventButton *event)
{
	...
	INPUT_TRACE("event type=%d button=%u state=0x%x x=%.1f y=%.1f item=%s selected=%d action=%d",
		...
		item ? (const char *) item->leafname : "<background>",
		item ? view_get_selected(view, &iter) : 0, action);
	...
	switch (action)
	{
		...
		case ACT_EDIT_ITEM:
			...
			/* (no break) */
		case ACT_OPEN_ITEM:
			INPUT_TRACE("open item requested: %s",
				item ? (const char *) item->leafname : "<none>");
			...
			break;
		case ACT_POPUP_MENU:
			INPUT_TRACE("popup menu requested: item=%s",
				item ? (const char *) item->leafname : "<background>");
			dnd_motion_ungrab();
			tooltip_show(NULL);
			show_filer_menu(filer_window,
...
static gboolean tooltip_activate(GtkWidget *window)
{
	...
	rox_gdk_window_get_pointer(motion_window, &x, &y, NULL);
...
void filer_save_settings(FilerWindow *fwin)
{
	...
	*/
	...
	dialog_add_icon_button(GTK_DIALOG(set_win->window), ROX_ICON_OK,
