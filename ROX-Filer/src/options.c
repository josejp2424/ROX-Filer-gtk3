...
/* Modificado por josejp2424: tooltips públicos y nativos de GTK3. */
#define OPTION_TIP(widget, tip) gtk_widget_set_tooltip_text((widget), (tip))
				  ...
				  guchar *label, GtkAdjustment *adj)
{
	...
	if (label)
	{
		...
		gtk_widget_set_halign(label_wid, GTK_ALIGN_END);
		gtk_widget_set_valign(label_wid, GTK_ALIGN_CENTER);
...
static GtkWidget *current_fontsel_box = NULL;
...
static void font_chosen(GtkWidget *dialog, gint response, Option *option)
{
	...
	/* Modificado por josejp2424: GtkFontChooser es la API pública GTK3. */
...
static void toggle_active_font(GtkToggleButton *toggle, Option *option)
{
	...
	/* GtkWidget structs are opaque in GTK3; use the public accessor. */
	GtkWidget *parent = gtk_widget_get_parent(option->widget);
...
static void open_fontsel(GtkWidget *button, Option *option)
{
	...
	if (current_fontsel_box)
		gtk_widget_destroy(current_fontsel_box);
	...
	if (option->value[0] != '\0')
		gtk_font_chooser_set_font(GTK_FONT_CHOOSER(current_fontsel_box),
					  option->value);

	g_signal_connect(current_fontsel_box, "response",
			G_CALLBACK(font_chosen), option);

	gtk_widget_show(current_fontsel_box);
}
...
static void build_menu_item(xmlNode *node, GtkComboBoxText *combo)
{
	guchar *label;
	guchar *value;
	...
	if (label && value)
	{
		/* Use a GTK3 text combo for enumerated options. */
		gtk_combo_box_text_append(combo, (const gchar *) value,
					  _((const gchar *) label));
	}
	xmlFree(label);
	xmlFree(value);
...
static void build_widget(xmlNode *widget, GtkWidget *box)
{
	...
	if (strcmp(name, "hbox") == 0 || strcmp(name, "vbox") == 0)
	{
		...
		if (name[0] == 'h')
			nbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 4);
		else
			nbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
			  ...
			  GtkTreeStore *tree_store, GtkTreeIter *parent)
{
	...
	/* GTK3: each options page must scroll independently.  Without this,
	 * the largest page forces the whole Options window to its natural
	 * width, which can span almost the entire monitor. */
	...
	gtk_tree_store_set(tree_store, &iter, 0, _(title), 1, scroller, -1);
	g_free(title);
...
{
	/* The colour chooser is embedded in the options page in GTK3, so there
	 * is no separate colour-selection dialog to destroy here. */
	if (current_fontsel_box)
		gtk_widget_destroy(GTK_WIDGET(current_fontsel_box));
...
{
	GtkWidget	*notebook;
	GtkWidget	*tl_vbox, *hbox, *frame, *tv, *tree_scroll;
	...
	tl_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 4);
	...
	gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(tree_scroll),
				    GTK_SHADOW_NONE);
	gtk_scrolled_window_set_propagate_natural_width(
		GTK_SCROLLED_WINDOW(tree_scroll), FALSE);
	gtk_scrolled_window_set_propagate_natural_height(
		GTK_SCROLLED_WINDOW(tree_scroll), FALSE);
	gtk_widget_set_size_request(tree_scroll, 185, -1);
	gtk_container_add(GTK_CONTAINER(tree_scroll), tv);
	gtk_container_add(GTK_CONTAINER(frame), tree_scroll);
	...
	revert_widget = button_new_mixed(ROX_ICON_UNDO, _("_Revert"));
	gtk_widget_set_can_default(revert_widget, TRUE); /* Modificado por josejp2424: GTK3 */
...
static void option_menu_set(GtkComboBox *combo, const guchar *value)
{
	/* Modificado por josejp2424: IDs estables en GtkComboBox GTK3. */
	if (!gtk_combo_box_set_active_id(combo, (const gchar *) value))
		gtk_combo_box_set_active(combo, 0);
}

/* Get the value (static) of the selected item. */
static const guchar *option_menu_get(GtkComboBox *combo)
{
	return (const guchar *) gtk_combo_box_get_active_id(combo);
}
...
{
	option_menu_set(GTK_COMBO_BOX(option->widget), option->value);
}
...
static void update_font(Option *option)
{
	...
	if (active)
	{
		...
		if (parent)
			gtk_widget_set_sensitive(parent, have_font);
	}
...
static void update_colour(Option *option)
{
	...
	if (!gdk_rgba_parse(&colour, option->value))
		gdk_rgba_parse(&colour, "#000000");
	gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(option->widget), &colour);
}
...
{
	return g_strdup_printf("%d", (int) gtk_adjustment_get_value(
		gtk_range_get_adjustment(GTK_RANGE(option->widget))));
}
...
{
	const guchar *value = option_menu_get(GTK_COMBO_BOX(option->widget));

	return g_strdup(value ? value : (const guchar *) "");
}
...
static guchar *read_colour(Option *option)
{
	...
	gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(option->widget), &colour);
...
static GList *build_label(Option *option, xmlNode *node, guchar *label)
{
	...
	gtk_widget_set_valign(widget, help ? GTK_ALIGN_CENTER : GTK_ALIGN_END);
	gtk_label_set_justify(GTK_LABEL(widget), GTK_JUSTIFY_LEFT);
	gtk_label_set_line_wrap(GTK_LABEL(widget), TRUE);

	if (help)
	{
		GtkWidget *hbox, *image, *spacer;
...
static GList *build_slider(Option *option, xmlNode *node, guchar *label)
{
	...
	{
		label_wid = gtk_label_new(_(label));
		gtk_widget_set_halign(label_wid, GTK_ALIGN_START);
		gtk_widget_set_valign(label_wid, GTK_ALIGN_CENTER);
		gtk_box_pack_start(GTK_BOX(hbox), label_wid, FALSE, TRUE, 0);
		add_to_size_group(node, label_wid);
	}
	...
	slide = gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, adj);
	...
	gtk_widget_set_can_focus(slide, FALSE); /* Modificado por josejp2424: GTK3 */
...
static GList *build_radio_group(Option *option, xmlNode *node, guchar *label)
{
	...
	if (cols > 1)
	{
		...
		/* Lay out option rows with GtkGrid. */
		grid = gtk_grid_new();
...
static GList *build_colour(Option *option, xmlNode *node, guchar *label)
{
	GtkWidget *hbox, *button, *label_wid;
	...
	if (label)
	{
		label_wid = gtk_label_new(_((const gchar *) label));
		gtk_widget_set_halign(label_wid, GTK_ALIGN_END);
		gtk_widget_set_valign(label_wid, GTK_ALIGN_CENTER);
		gtk_box_pack_start(GTK_BOX(hbox), label_wid, TRUE, TRUE, 0);
	}

	/* Modificado por josejp2424: selector de color nativo de GTK3. */
...
static GList *build_menu(Option *option, xmlNode *node, guchar *label)
{
	GtkWidget *hbox, *option_menu, *label_wid;
	xmlNode *item;
	...
	/* Read the selected value from GtkComboBoxText. */
	...
	for (item = node->xmlChildrenNode; item; item = item->next)
	{
		if (item->type == XML_ELEMENT_NODE)
			build_menu_item(item, GTK_COMBO_BOX_TEXT(option_menu));
	}
...
static GList *build_font(Option *option, xmlNode *node, guchar *label)
{
	...
	option->widget = gtk_label_new("");
	button = gtk_button_new();
	gtk_container_add(GTK_CONTAINER(button), option->widget);
	gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, TRUE, 0);
