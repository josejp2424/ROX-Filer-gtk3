...
#include "options.h"
#include "cell_icon.h"
/* Detail view uses GtkCellRendererText directly. */
...
/* Static prototypes */
static void view_details_finalize(GObject *object);
...
GtkWidget *view_details_new(FilerWindow *filer_window)
{
	...
	gtk_range_set_adjustment(GTK_RANGE(filer_window->scrollbar),
		gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(view_details)));
...
typedef enum
{
	DETAILS_SELECTION_SET,
	DETAILS_SELECTION_INVERT
} DetailsSelectionOp;
...
static gboolean select_lasso_cb(ViewIter *iter, gpointer data)
{
	...
	int end = ((int *) data)[1];
	DetailsSelectionOp op = ((int *) data)[2];
...
static void select_lasso(ViewDetails *view_details, DetailsSelectionOp op)
{
	GtkAdjustment	*adj;
	int range[3];

	adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(view_details));

	range[0] = view_details->lasso_start_index;
	range[1] = get_lasso_index(view_details,
			      view_details->drag_box_y[1] - (gint) gtk_adjustment_get_value(adj));
	range[2] = op;
					    ...
					    GdkEventButton *bev)
{
	...
	if (motion_buttons_pressed == 0 && view_details->lasso_box)
	{
		select_lasso(view_details,
				bev->button == 1 ? DETAILS_SELECTION_SET
					 : DETAILS_SELECTION_INVERT);
...
static gint view_details_motion_notify(GtkWidget *widget, GdkEventMotion *event)
{
	...
	{
		GtkAdjustment	*adj;
		adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(tree));
...
static void view_details_draw_outline(GtkWidget *widget, cairo_t *cr,
		const GdkRectangle *area)
{
	...
	gtk_style_context_get_color(context, GTK_STATE_FLAG_NORMAL, &colour);

	cairo_save(cr);
	gdk_cairo_set_source_rgba(cr, &colour);
	cairo_set_line_width(cr, 1.0);
	cairo_rectangle(cr, area->x + 0.5, area->y + 0.5,
		MAX(0, area->width - 1), MAX(0, area->height - 1));
	cairo_stroke(cr);
	cairo_restore(cr);
}

static gboolean view_details_draw(GtkWidget *widget, cairo_t *cr)
{
	GtkTreeView *tree = GTK_TREE_VIEW(widget);
	GtkTreePath *path = NULL;
	GdkRectangle focus_rectangle;
	GdkWindow *bin_window;
	ViewDetails *view_details = VIEW_DETAILS(widget);
	GtkWidgetClass *parent = GTK_WIDGET_CLASS(parent_class);
	gboolean handled = FALSE;

	if (parent->draw)
		handled = parent->draw(widget, cr);

	bin_window = gtk_tree_view_get_bin_window(tree);
	if (!bin_window || !gtk_cairo_should_draw_window(cr, bin_window))
		return handled;

	if (!view_details->filer_window || !view_details->filer_window->directory)
		return handled;

	/* GtkTreeView owns several GdkWindows.  The draw context starts in
	 * widget coordinates; the old expose handler used bin-window coordinates.
	 */
	cairo_save(cr);
	...
	if (view_details->filer_window->directory->error)
	{
		...
		layout = gtk_widget_create_pango_layout(widget,
			view_details->filer_window->directory->error);
	...
	if (view_details->lasso_box)
	{
		...
		adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(tree));
		area.x = MIN(view_details->drag_box_x[0],
			view_details->drag_box_x[1]);
		area.y = MIN(view_details->drag_box_y[0],
			view_details->drag_box_y[1]) -
			(gint) gtk_adjustment_get_value(adj);
		area.width = abs(view_details->drag_box_x[1] -
			view_details->drag_box_x[0]);
		area.height = abs(view_details->drag_box_y[1] -
			view_details->drag_box_y[0]);
		view_details_draw_outline(widget, cr, &area);
	}
	...
	if (view_details->wink_item != -1 && view_details->wink_step & 1)
	{
		...
		wink_path = gtk_tree_path_new_from_indices(
			view_details->wink_item, -1);
		gtk_tree_view_get_background_area(tree, wink_path,
			NULL, &wink_area);
		gtk_tree_path_free(wink_path);

		if (wink_area.height > 0)
		{
			gtk_widget_get_allocation(widget, &allocation);
			wink_area.x += 1;
			wink_area.y += 1;
			wink_area.width = MAX(0, allocation.width - 3);
			wink_area.height = MAX(0, wink_area.height - 3);
			view_details_draw_outline(widget, cr, &wink_area);
		}
	}
	...
	if (focus_rectangle.height > 0)
	{
		...
		gtk_render_focus(context, cr,
			focus_rectangle.x, focus_rectangle.y,
			focus_rectangle.width, focus_rectangle.height);
	}
...
static void view_details_get_preferred_width(GtkWidget *widget,
		gint *minimum_width, gint *natural_width)
{
	...
	if (parent->get_preferred_width)
		parent->get_preferred_width(widget, &minimum, &natural);

	view_details->desired_size.width = natural;
...
static void view_details_get_preferred_height(GtkWidget *widget,
		gint *minimum_height, gint *natural_height)
{
	...
	if (parent->get_preferred_height)
		parent->get_preferred_height(widget, &minimum, &natural);

	view_details->desired_size.height = natural;
...
static void view_details_destroy(GtkWidget *widget)
{
	ViewDetails *view_details = VIEW_DETAILS(widget);
	GtkWidgetClass *parent = GTK_WIDGET_CLASS(parent_class);
...
static void view_details_finalize(GObject *object)
{
	ViewDetails *view_details = VIEW_DETAILS(object);

	if (view_details->items)
	{
...
static void view_details_class_init(gpointer gclass, gpointer data)
{
	...
	widget->destroy = view_details_destroy;
	...
	widget->draw = view_details_draw;
	widget->get_preferred_width = view_details_get_preferred_width;
	widget->get_preferred_height = view_details_get_preferred_height;
...
static void view_details_autosize(ViewIface *view)
{
	...
	gtk_widget_get_preferred_size(GTK_WIDGET(view), &minimum, &natural);
...
static void set_lasso(ViewDetails *view_details, int x, int y)
{
	...
	if (area.width && area.height)
	{
		...
		adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(view_details));
		area.y -= (gint) gtk_adjustment_get_value(adj);
...
static gboolean view_details_auto_scroll_callback(ViewIface *view)
{
	...
	h = (gint) gtk_adjustment_get_page_size(adj);
	...
	if (diff)
	{
		gdouble	old = gtk_adjustment_get_value(adj);
		gdouble	value = old + diff;
