...
void bookmarks_show_menu(FilerWindow *filer_window)
{
	...
	GtkMenu *menu = GTK_MENU(bookmarks_build_menu(filer_window));
...
void bookmarks_edit(void)
{
	...
	for (node = node->xmlChildrenNode; node; node = node->next)
	{
		...
		if (node->type != XML_ELEMENT_NODE)
			continue;
		if (xmlStrcmp(node->name, BAD_CAST "bookmark") != 0)
			continue;
		...
		gtk_list_store_set(model, &iter, 0, (const gchar *) mark, 1, (const gchar *) title, -1);
		if(title!=mark)
			xmlFree(title);
...
static void bookmarks_add_dir(const guchar *dir)
{
	...
	bookmark = xmlNewTextChild(xmlDocGetRootElement(bookmarks->doc),
				NULL, BAD_CAST "bookmark", dir);
	xmlSetProp(bookmark, BAD_CAST "title", dir);
...
static void bookmarks_activate(GtkMenuShell *item, FilerWindow *filer_window)
{
	...
	if (!mark && GTK_IS_MENU_ITEM(item))
	{
		GtkWidget *label = menu_item_get_label_widget(GTK_WIDGET(item));
		mark = label ? gtk_label_get_text(GTK_LABEL(label))
			    ...
			    guint time, GtkTreeView *view)
{
	...
	uris = uri_list_to_glist((const char *) sd_data);
...
static void commit_edits(GtkTreeModel *model)
{
	...
	if (gtk_tree_model_get_iter_first(model, &iter))
	{
		...
		do
		{
			...
			bookmark = xmlNewTextChild(root, NULL, BAD_CAST "bookmark",
						  BAD_CAST g_value_get_string(&mark));
			g_value_unset(&mark);
			gtk_tree_model_get_value(model, &iter, 1, &title);
			xmlSetProp(bookmark, BAD_CAST "title",
						   BAD_CAST g_value_get_string(&title));
			g_value_unset(&title);
...
static GtkWidget *bookmarks_build_menu(FilerWindow *filer_window)
{
	...
	for (node = node->xmlChildrenNode; node; node = node->next)
	{
		...
		if (!mark)
			continue;
		path = g_strdup((const gchar *) mark);
